...
enddate= "".join(str(dt_now.date()).split('-'))
#print(enddate)
startdate="20200120"
...
for item in items:
    ...
    stateDt_list.append(stateDt.get_text())
    decideCnt=item.select_one('decideCnt') # 누적확진자 수를 태그로 받음
    decideCnt_list.append(int(decideCnt.get_text()))
    accExamCnt=item.select_one('accExamCnt') # 누적 검사자 수를 태그로 받음
    accExamCnt_list.append(int(accExamCnt.get_text()))
    deathCnt=item.select_one('deathCnt') # 누적사망자 수를 태그로 받음
    deathCnt_list.append(int(deathCnt.get_text()))
    
# 일일 확진자수,검사자수,사망자수 계산
for data in range(len(stateDt_list)-1):
    daily_decideCnt_list.append(decideCnt_list[data]-decideCnt_list[data+1]) # 일일 확진자수 리스트 생성
...
daily_decideCnt_list.append(decideCnt_list[len(stateDt_list)-1]) # 처음의 일일 확진자수 리스트에 추가
daily_accExamCnt_list.append(accExamCnt_list[len(stateDt_list)-1]) # 처음의 일일 검사자수 리스트에 추가
daily_deathCnt_list.append(deathCnt_list[len(stateDt_list)-1]) # 처음의 일일 사망자수 리스트에 추가
